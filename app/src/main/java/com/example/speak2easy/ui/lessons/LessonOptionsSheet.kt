...
fun LessonOptionsSheet(
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            // iOS parity: Auto-play is a sub-option of Hands-free.
            if (options.isHandsFree) {
                SubOptionToggle("Play pronunciation out loud", options.autoPlayAudio) {
                    options = options.copy(autoPlayAudio = it)
                }
            }
            OptionToggle("Focus mode", "Hide hints until you ask", options.isFocusMode) {
                options = options.copy(isFocusMode = it)
            }
...
@Composable
private fun SubOptionToggle(title: String, checked: Boolean, onChange: (Boolean) -> Unit) {
    val c = SonariTheme.colors
    Row(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Text(title, style = SonariFonts.monoTiny, color = c.textPrimary, modifier = Modifier.weight(1f))
        Switch(
            checked = checked,
